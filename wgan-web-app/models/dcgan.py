...
class DCGAN_MODEL(object):
    def __init__(self, is_train, download, dataroot, dataset, generator_iters, cuda, batch_size, load_D, load_G):
    ...
    def evaluater(self, test_loader, D_model_path, G_model_path):
        self.load_model(D_model_path, G_model_path)
        z = Variable(torch.randn(64, 100, 1, 1)).cuda(self.cuda_index)
        samples = self.G(z)
        samples = samples.mul(0.5).add(0.5)
        samples = samples.data.cpu()
        grid = utils.make_grid(samples)
        print("Grid of 1 images saved to 'dgan_model_image.png'.")
        utils.save_image(grid, 'static/images/dgan_model_image.png')
